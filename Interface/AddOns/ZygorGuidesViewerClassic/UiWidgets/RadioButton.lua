...
function RadioButton:New(parent)
	local button = CHAIN(ui:Create("Button",parent))
		:SetBackdrop(SkinData("RadioOn"))
		--:SetBackdropColor(0,0,0,0)
		:SetBackdropBorderColor(0,0,0,0)
		:SetSize(DEFAULTWIDTH,DEFAULTHEIGHT)
		:SetScript("OnEnter",function()
			if not nocallbacks and button and button.onEnterCallbacks then
		...
		:SetScript("OnLeave",function()
			if not nocallbacks and button and button.onLeaveCallbacks then
	...
	button.hilitex = CHAIN(button:CreateTexture())
		:SetAllPoints()
		:SetColorTexture(unpack(SkinData("ToggleButtonHighlightColor")))
	.__END
	...
	button.textbutton = CHAIN(CreateFrame("Button",nil,button))
		...
		:SetScript("OnEnter",function()
			...
			if not nocallbacks and button.onEnterCallbacks then
				for _, callback in pairs(button.onEnterCallbacks) do
					callback(button)
				end
			end
		...
		:SetScript("OnLeave",function()
			...
			if not nocallbacks and button.onLeaveCallbacks then
				for _, callback in pairs(button.onLeaveCallbacks) do
					callback(button)
				end
			end
...
function RadioButton:Toggle(nocallbacks)
	...
	self:SetBackdrop(SkinData("RadioOn"))
